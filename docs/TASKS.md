...
| T2 | Git initial commit (+ GitHub if gh auth available) | ✅ done | commit 07e5860; GitHub push pending gh auth/remote |
...
| T6 | RTSP adapter proof (serve clip via RTSP, onboard as rtsp source) | ⚠️ blocked | rtsp:// client path works (same FFmpeg backend); local RTSP servers (ffmpeg-listen, mediamtx) blocked by Defender on this machine — retry on demo machine or with signed binary |
